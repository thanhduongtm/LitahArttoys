...
(function ($) {
    ...
    var
        ...
        $collectionCartModalLinkLq = $('[data-modal="modal-lq"]'),
        ...
        $modalProductDetailElementLq = $('#js-product-detail-modal-lq'),

        $modalProductDetailElementThumbnail = $('#js-product-detail-modal-thumbnail'),
        $modalProductDetailElementThumbnailLq = $('#js-product-detail-modal-thumbnail-lq'),
    ...
    RESHOP.initModalLq = function() {
        // Check if these anchors are on page
        console.log("initModalLq")
        if ($collectionCartModalLinkLq.length) {
            $collectionCartModalLinkLq.on('click',function () {
                var getElemId = $(this).data('modal-id-lq');
                $(getElemId).modal({
                    backdrop: 'static',
                    keyboard: false,
                    show:true
    ...
    // Blog Post Embed Video - KHÔNG XÓA
    ...
    RESHOP.modalProductDetailInit = function() {
        if ($modalProductDetailElement.length && $modalProductDetailElementThumbnail.length) {
            ...
            $('#quick-look-mc').on('shown.bs.modal', function () {
    ...
    RESHOP.modalProductDetailInitLq = function() {
        console.log("lien quan")
        if ($modalProductDetailElementLq.length && $modalProductDetailElementThumbnailLq.length) {
            $modalProductDetailElementLq.slick({
                slidesToShow: 1,
                slidesToScroll: 1,
                infinite:false,
                arrows: false,
                dots: false,
                fade: true,
                asNavFor: $modalProductDetailElementThumbnailLq
            });

            $modalProductDetailElementThumbnailLq.slick({
                slidesToShow: 4,
                slidesToScroll: 1,
                infinite:false,
                arrows: true,
                dots: false,
                focusOnSelect: true,
                asNavFor: $modalProductDetailElementLq,
                prevArrow:'<div class="pt-prev"><i class="fas fa-angle-left"></i>',
                nextArrow:'<div class="pt-next"><i class="fas fa-angle-right"></i>',
                responsive: [
                    {
            ...
            // Hook into Bootstrap shown event and manually trigger 'resize' event
            // so that Slick recalculates the widths
            $('#quick-look-lq').on('shown.bs.modal', function () {
                $modalProductDetailElementLq.resize();
            });
        }
    };
    ...
    $(window).on('resize', function() {
        if ($modalProductDetailElement.length && $modalProductDetailElement.is(':visible')) {
            $modalProductDetailElement.slick('setPosition');
        }
        if ($modalProductDetailElementLq.length && $modalProductDetailElementLq.is(':visible')) {
            $modalProductDetailElementLq.slick('setPosition');
        }
        if ($modalProductDetailElementThumbnail.length && $modalProductDetailElementThumbnail.is(':visible')) {
            $modalProductDetailElementThumbnail.slick('setPosition');
        }
        if ($modalProductDetailElementThumbnailLq.length && $modalProductDetailElementThumbnailLq.is(':visible')) {
            $modalProductDetailElementThumbnailLq.slick('setPosition');
        }
    }).trigger('resize');
        ...
        RESHOP.modalProductDetailInitLq();
